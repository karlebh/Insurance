<template>
  <header class="lg:hidden font-poppins">
    <div :class="{'shadow-xl': !isOpen}" 
    class="h-20 bg-gray-50 flex items-center justify-between px-4 py-14 items-center z-40 top-0" id="mobileHeader">
    ...
    <button @click="isOpen = !isOpen">
      <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32">
        <g fill="none" fill-rule="evenodd">
          <path fill="#FFF" stroke="#2C2830" stroke-width="1.5" d="M.75.75h30.5v30.5H.75z"/><g fill="#2C2830"><path d="M8 10h16v1.5H8zM8 15h16v1.5H8zM8 20h16v1.5H8z"/>
          </g></g></svg>
        </button>
      </div>
      <NavBar v-if="isOpen" :isOpen="isOpen" @toggle="isOpen = !isOpen"/>

      <div class="hero" id="hero">
        <img class="w-full object-fill object-center h-full" src="@/assets/images/image-intro-mobile.jpg" alt="">
      </div>
    </header>
  </template>
      ...
      mounted() {
        window.onscroll = function() {
          if (window.pageYOffset >= 600) {
            document.getElementById('mobileHeader').classList.add('fixed', 'w-full')
          } else {
            document.getElementById('mobileHeader').classList.remove('fixed', 'w-full')
          }
        }
      },
      components: {
